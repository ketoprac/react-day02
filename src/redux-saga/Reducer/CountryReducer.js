...
const DelCountrySuccessed = (state, action) => {
  ...
  const filterRegion = state.countries.filter((el) => el.countryId !== payload);
  return {
    ...state,
...
const EditCountrySuccessed = (state, action) => {
  const { payload } = action;
  const filterRegion = state.countries.filter(
    (el) => el.countryId !== payload[0].countryId
  );
  return {
    ...state,
    countries: [...filterRegion, payload[0]],
  };
};
